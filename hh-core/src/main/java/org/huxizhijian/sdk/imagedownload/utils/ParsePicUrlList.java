...
public class ParsePicUrlList {
    ...
    public static ArrayList<String> scanPicInPage(int serverId, String contents) {                        //获取图片网址列表
        ...
        picList = parsePicUrl(picListUrl, ImageDownloader.getInstance().getEncodeKey(), serverId);
        return picList;
    }
    ...
    private static ArrayList<String> parsePicUrl(String picListUrl, String keyWord, int serverId) {           //分析隐藏的图片网址,根据网站的js代码获取网址
        ...
        if (serverId < 10) {
            serverUrl = ImageDownloader.getInstance().getPicServer() + "0" + serverId + "/";
        } else {
            serverUrl = ImageDownloader.getInstance().getPicServer() + serverId + "/";
        }
